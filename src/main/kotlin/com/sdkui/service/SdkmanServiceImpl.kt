...
class SdkmanServiceImpl(
    ...
    override suspend fun listCandidates(): Result<List<Sdk>> =
        withContext(Dispatchers.IO) {
            runCatching { parseCandidates(runSdk("list")) }
        }

    override suspend fun getCurrentDefaults(): Result<Map<String, String>> =
        withContext(Dispatchers.IO) {
            runCatching { parseDefaults(runSdk("current")) }
        }

    override suspend fun checkForUpdate(): Result<SdkmanUpdateStatus> =
        withContext(Dispatchers.IO) {
            runCatching {
    ...
    override suspend fun setDefault(
        ...
        withContext(Dispatchers.IO) {
            runCatching {
                runSdk("default", candidate, identifier)
                Unit
            }
        }

    override fun selfUpdate(): Flow<String> =
        flow {
            val proc =
                ProcessBuilder("/bin/bash", "-c", "source $sdkmanInit && sdk selfupdate")
    ...
    override fun install(
        ...
        flow {
            ...
            val proc =
                ProcessBuilder("/bin/bash", "-c", "source $sdkmanInit && sdk install '$candidate'$versionArg")
                    .redirectErrorStream(true).start()
            proc.outputStream.close()
            proc.inputStream.bufferedReader().useLines { it.forEach { line -> emit(line) } }
            proc.waitFor()
    ...
    override fun uninstall(
        ...
        flow {
            val proc =
                ProcessBuilder("/bin/bash", "-c", "source $sdkmanInit && sdk uninstall '$candidate' '$identifier'")
                    .redirectErrorStream(true).start()
            proc.inputStream.bufferedReader().useLines { it.forEach { line -> emit(line) } }
            proc.waitFor()
        }.flowOn(Dispatchers.IO)
        ...
        fun parseCandidates(raw: String): List<Sdk> {
            ...
            fun flushBlock() {
                ...
                if (name != null) {
                    ...
                    val description =
                        if (headerIdx >= 0 && installIdx > headerIdx) {
